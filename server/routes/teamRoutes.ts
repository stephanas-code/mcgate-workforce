...
// 4. Generate next auto-assigned Employee Code
router.get('/employees/next-code', authenticateToken, async (_req, res) => {
  try {
    const rows = await queryAll<{ employee_code: string }>('SELECT employee_code FROM employees');
    let maxNum = 0;
    for (const r of rows) {
...
// 5. Update Teammate / Employee Avatar (Allow teammates or admins to upload profile picture)
...
// 7. Create new Employee + User (Admins / HR only) with auto-assigned code
router.post('/employees', authenticateToken, requireRoles('SUPER_ADMIN', 'ADMIN'), async (req: AuthRequest, res) => {
  try {
    ...
    // Normalize and validate corporate role
    ...
    // Verify existing user / cleanup orphaned records
    const existingUser = await queryOne<{ id: number }>('SELECT id FROM users WHERE email = ?', [cleanEmail]);
    if (existingUser) {
      const existingEmp = await queryOne<{ id: number }>('SELECT id FROM employees WHERE user_id = ?', [existingUser.id]);
      if (existingEmp) {
        return res.status(400).json({ error: 'A user with this corporate email address already exists.' });
      }
      // If orphaned user with no employee record, remove it to allow clean registration
      await execute('DELETE FROM users WHERE id = ?', [existingUser.id]);
    }

    // Auto-generate or validate unique employee code
    ...
    if (!finalEmpCode) {
      ...
      // If code was supplied, ensure no duplicate exists; otherwise auto-assign
    ...
    `, [
      userId, finalEmpCode, cleanFirst, cleanLast, avatarUrl || null, phone || '', cleanJobTitle,
